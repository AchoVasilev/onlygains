...
import com.project.application.models.workout.CreateSetResource
import com.project.application.models.workout.CreateWorkoutExerciseResource
import com.project.application.models.workout.SetResource
...
@Singleton
open class WorkoutService(private val workoutRepository: WorkoutRepository,
    ...
    open fun addExerciseToWorkout(workoutId: UUID, newExercise: CreateWorkoutExerciseResource): WorkoutDetailsResource {
        ...
        if (newExercise.sets.isEmpty()) {
            ...
            val sets = newExercise.sets.map { s: CreateSetResource -> WorkoutSet.from(s.weight, s.repetitions) }
    ...
    private fun findById(id: UUID): Workout {
        return workoutRepository.findById(id)
                .orElseThrow { EntityNotFoundException(Workout::class.java, id) }
    }
